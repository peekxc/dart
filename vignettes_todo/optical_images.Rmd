...
precompute_moves <- function(path, minimal_bigrades_t){
	...
	for (i in 2:nrow(path)){
		...
		## Records the schedules 
		c_stops <- attr(M1[[bv]], "stops")
		M1[[bv]] <- cbind(M1[[bv]], S1)
		attr(M1[[bv]], "stops") <- c(c_stops, ncol(S1))
		
		c_stops <- attr(M2[[bv]], "stops")
		M2[[bv]] <- cbind(M2[[bv]], S2)
		attr(M2[[bv]], "stops") <- c(c_stops, ncol(S2))
		
		## Move or reset to get next vertex
		if (v %in% needs_saved){
...
for (v in names(M1)){
	...
	fi <- filtration(S$complex, grades = P[slex_order,3])
	rv <- reduce(boundary_matrix(fi, dims = 1:2))
	nc <- simulate_moves(rv, schedule = M1[[v]], dim = 1L)
	nc <- nc + simulate_moves(rv, schedule = M2[[v]], dim = 2L)
	moves_nc <- moves_nc + nc
}
```
...
<!-- #S <- sparse_rips_filtration(dx, alpha = max_diam, dim = 2L, epsilon = 0.0) -->
<!-- # sr <- sparse_rips(dx, alpha = max_diam) -->
<!-- # S <- sparsify_rips(sr$graph, p = sr$permutation, w = sr$radii, epsilon = 0.1,  -->
<!-- # 									 alpha = sr$alpha, filtered = TRUE, dim = 2) -->
<!-- # S <- sparse_rips_filtration(x, epsilon = 0.1, dim = 2L) -->
...
x <- X[sample_idx,,drop=FALSE]
dx <- geodesic_dist(x, k = 15L)
S <- rips_filtration(dx, diameter = max_diam, dim = 2) 
f1 <- normalize(S$grading)
f2 <- normalize(sapply(S$simplices, function(simplex){ max(knn_density[sample_idx[simplex]]) }))

nc_totals <- matrix(0, nrow = 0, ncol = 4, dimnames=list(NULL, c("bin_sizes", "r", "v", "m")))
	
for (b in bin_sizes){
	xbin <- b; ybin <- b
	
	## Get line arrangement + dual graph 
	ro <- bigraded_betti2(x = S$simplices, f1 = f1, f2 = f2, H = 1L, xbin = xbin, ybin = ybin, save.path = "~/pc_f.txt")
	...
	minimal_bigrades_t <- t(structure(
		cbind(f1, f2), 
		dimnames = list(unlist(simplex_to_str(S$simplices)), NULL))
	)
	...
	## Deduce from the path which decompositions to save in memory
	needs_saved <- !sapply(2:nrow(path), function(i){ path[i,1] == path[i-1,2] })
	needs_saved <- path[c(FALSE, needs_saved),1]
	saved <- list()
	
	## Get the fiber for the first filtration
	bv <- as.character(e0) ## base vertex
	P <- dart:::push_map(minimal_bigrades_t, m = fibers[e0,1], b = fibers[e0,2])
	p <- order(P[,3])
	rv_base <- reduce(boundary_matrix(filtration(S$complex, grades = p)))
	
	## Measure operations for naive reductions 
	reduce_nc <- 0L
	{
		for (v in vertex_path){
			bigrades <- dart:::push_map(minimal_bigrades_t, m = fibers[v,1], b = fibers[v,2])
			fi <- filtration(S$complex, bigrades[,3])
			rv <- reduce(boundary_matrix(fi, dims = 1:2), show_progress = FALSE)
			reduce_nc <- reduce_nc + rv$nc
		}
	}
	
	## Measure operations for vineyard updates 
	vineyard_nc <- 0L
	R_psp <- rv_base$R
	V_psp <- rv_base$V
	{
		for (i in 2:nrow(path)){
			...
			q <- order(Q[,3])
			schedule <- local({
				sch <- vineyards_schedule(S$simplices[p], S$simplices[q])
				if (is.matrix(sch)){ return(sch) }
				pmin(sch$schedule[1,], sch$schedule[2,])
			})
	
			vineyard_nc <- vineyard_nc + dart:::simulate_vineyard_pspbool(
				R_ptr = R_psp$matrix$as_XPtr(), V_ptr = V_psp$matrix$as_XPtr(), 
				schedule = schedule - 1L, show_progress = FALSE)
			if (v %in% needs_saved){
				print(sprintf("Saving: %d", v))
				saved[[as.character(v)]] <- fibers[v,]
			}
			if (i != nrow(path)){
				if (path[i,2] != path[i+1,1]){
					...
					P <- dart:::push_map(minimal_bigrades_t, m = c_fiber[1], b = c_fiber[2])
					p <- order(P[,3])
					fi <- filtration(S$complex, grades = P[,3])
					D <- boundary_matrix(fi)
					R_psp <- psp_matrix(x=D$matrix)
					V_psp <- dart::psp_matrix(x = Matrix::Diagonal(ncol(D$matrix)))
					## Don't count resets 	
					nc_local <- dart:::reduce_pspbool(R_psp$matrix$as_XPtr(), V_psp$matrix$as_XPtr(), show_progress = TRUE)
					# vineyard_nc <- vineyard_nc
				} else {
					p <- q
				}
			}
		}
	} ## end vineyards updates 
	
	## Start move updates 
	moves_nc <- 0L
	{
		schedule <- precompute_moves(path, minimal_bigrades_t) 
		slex_order <- match(
			ltraverse(level_order(S$complex), as.integer),
			unname(lapply(str_to_simplex(colnames(minimal_bigrades_t)), as.integer)) 
		)
		for (v in names(schedule$M1)){
			vi <- as.integer(v)
			P <- dart:::push_map(x = minimal_bigrades_t, m = fibers[vi,1], b = fibers[vi,2])
			fi <- filtration(S$complex, grades = P[slex_order,3])
			## don't count these; in real situation would not recompute but save them along path
			rv <- reduce(boundary_matrix(fi, dims = 1:2), show_progress = FALSE) 
			nc <- simulate_moves(rv, schedule = schedule$M1[[v]], dim = 1L)
			nc <- nc + simulate_moves(rv, schedule = schedule$M2[[v]], dim = 2L)
			moves_nc <- moves_nc + nc
		}
	}
	
	## Save totals 
	nc_totals <- rbind(nc_totals, c(b, reduce_nc, vineyard_nc, moves_nc))
	print(nc_totals)
}
```


```{r Extracting the fibered barcode}
schedule <- precompute_moves(path, minimal_bigrades_t) 
slex_order <- match(
	ltraverse(level_order(S$complex), as.integer),
	unname(lapply(str_to_simplex(colnames(minimal_bigrades_t)), as.integer)) 
)
if (any(is.na(slex_order))){ stop("bad") }
...
## Prepre template points
template_points <- structure(vector("list", length(vertex_path)), names = vertex_path)

## Save initial vertex
v <- vertex_path[1]
P <- dart:::push_map(x = minimal_bigrades_t, m = fibers[v,1], b = fibers[v,2])
fi <- filtration(S$complex, grades = P[slex_order,3])
rv <- reduce(boundary_matrix(fi, dims = 1:2), show_progress = FALSE) 
template_points[[as.character(v)]] <- read_barcodes(rv, dims = 1L, essential = TRUE)

p1 <- order(P[s1,3])
p2 <- order(P[s2,3])

P <- dart:::push_map(x = minimal_bigrades_t, m = fibers[2,1], b = fibers[2,2])
fi <- filtration(S$complex, P[slex_order,3])
q1 <- order(P[s1,3])
q2 <- order(P[s2,3])
S1 <- greedy_min_cross2(p1, q1, strategy = "pairwise")
S2 <- greedy_min_cross2(p2, q2, strategy = "pairwise")
simulate_moves(rv, schedule = S1, dim = 1L)
simulate_moves(rv, schedule = S2, dim = 2L)

## Validate decomposition under new filtration 
validate_decomp(rv, boundary_matrix(fi, dims = 1:2))

## Record the template points
bc2 <- read_barcodes(
	rv, dims = 1L, values = lapply(1:2, fi$k_grades), 
	essential = TRUE
)
...
for (v in vertex_path[-1]){
	...
	## Record the template points
	template_points[[as.character(v)]] <- read_barcodes(
		rv, dims = 1L, values = lapply(1:2, fi$k_grades), 
		essential = TRUE
	)
	
	## Reassign previous permutations
	p1 <- q1 
	p2 <- q2
}

simplices <- unname(lapply(str_to_simplex(colnames(minimal_bigrades_t)), as.integer)) 
P <- dart:::push_map(x = minimal_bigrades_t, m = fibers[5,1], b = fibers[5,2])
fi <- filtration(simplices, grades = P[,3])
rv <- reduce(boundary_matrix(fi, dims = 1:2), show_progress = FALSE) 
k_grades <- lapply(1:2, function(k){ fi$k_grades(k) })
bc <- read_barcodes(rv, dims = 1L, essential = FALSE, values = k_grades)
...
rv <- reduce(boundary_matrix(fi, dims = 1:2), show_progress = FALSE) 
bc2 <- read_barcodes(rv, dims = 1L, values = lapply(1:2, fi$k_grades), essential = TRUE)



head(read_barcodes(rv, dims = 1L, essential = TRUE, values = k_grades))
head(template_points[[as.character(5)]])

# P[match(as.list(faces(fi$complex, c(126,130,142))), simplices),]

fi_idx <- match(as.list(faces(fi$complex, c(126,130,142))), fi$simplices)
fi$grading[fi_idx]
P[match(as.list(faces(fi$complex, c(126,130,142))), simplices),3]

s_test <- list(1,2,3,c(1,2),c(2,3),c(1,3),c(1,2,3),4,c(2,4),c(1,4),c(1,2,4))
g_test <- seq_along(s_test)*0.10
fi <- filtration(s_test, g_test)
random_idx <- sample(seq_along(s_test))
fi <- filtration(s_test[random_idx], g_test[random_idx])

# sapply(simplices, function(simplex) any(is.na(simplex)))

bc2 <- read_barcodes(rv, dims = 1L, essential = TRUE, values = lapply(1:2, function(k){ fi$k_grades(k) }))
...
# 	
# 	
# 	stop_pts <- sort(attr(schedule$M1[[v]], "stops"))
# 	simulate_moves(rv, schedule = schedule$M1[[v]], dim = 1L, f = function(){
# 			
# 	})
# 	nc <- nc + simulate_moves(rv, schedule = schedule$M2[[v]], dim = 2L)
# }
...
## Just need to get the polygon/barcodes containing the point whose dual is L, then apply 
## push_L(a,b) for every (a,b) in the barcode template. Use the simplices + index persistence and projected order 
## to figure out the barcode template push map 
sf::st_intersects(sf::st_point(c(3.5, -0.5)), sf::st_collection_extract(LA, "POLYGON"), sparse=TRUE)[[1]]
```
