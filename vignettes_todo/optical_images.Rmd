...
ro <- bigraded_betti(x = x, f = f, H = 1L, xbin = 15L, ybin = 15L, max_dist = max_diam, 
										 save.path = "~/pc_f.txt")
...
S_dist <- apply(dg, 1, function(i){
  spearman_dist(fibered_filtrations[[i[1]]], fibered_filtrations[[i[2]]])
})
...
di <- unname(sapply(str_to_simplex(row.names(minimal_bigrades)), length)-1L)
s1 <- which(di==1)
s2 <- which(di==2)
bigrades <- dart:::push_map(t(minimal_bigrades), m = fibers[e0,1], b = fibers[e0,2])
p1 <- order(bigrades[s1,3])
p2 <- order(bigrades[s2,3])
for (i in 2:nrow(path)){
	v <- path[i,2]
	bigrades <- dart:::push_map(t(minimal_bigrades), m = fibers[v,1], b = fibers[v,2])
...
for (v in names(M1)){
	v <- names(M1)[[1]]
	D <- boundary_matrix(str_to_simplex(fibered_filtrations[[as.integer(v)]]), dim = c(1,2,3))
...
for (v in vertex_path){
	...
	ncol_reduction[match(v, vertex_path)] <- RV$nc
}
...
```{r}
## TODO: remove this in favor of dimension-specific moves
simplices <- unname(str_to_simplex(rownames(minimal_bigrades)))
saved <- list()
M <- structure(list(matrix(0, ncol = 0, nrow = 2)), names = e0)
bv <- as.character(e0) ## base vertex
p <- order(dart:::push_map(t(minimal_bigrades), m = fibers[e0,1], b = fibers[e0,2])[,3])
for (i in 2:nrow(path)){
	v <- path[i,2]
	q <- order(dart:::push_map(t(minimal_bigrades), m = fibers[v,1], b = fibers[v,2])[,3])
	S <- vineyards_schedule(simplices[p], simplices[q])
	M[[bv]] <- cbind(M[[bv]], S)
	if (v %in% needs_saved){
		print(sprintf("Saving: %d", v))
		saved[[as.character(v)]] <- q
	}
	if (i != nrow(path)){
		if (path[i,2] != path[i+1,1]){
			print("changing base vertex")
			bv <<- as.character(path[i+1,1]) ## change base vertex
			p <<- saved[[as.character(path[i+1,1])]]
		} else {
			p <<- q
		}
	}
