...
#' @examples 
#' library(simplextree)
#' all(unrank_lex(seq(choose(100,2)), n = 100, k = 2) == combn(100,2))
#' 
...
unrank_lex  <- function(x, n, k){
	...
	return(unrank_R(as.integer(x)-1L, n, k)+1L)
}
...
rank_lex <- function(x, n){
  ...
  if (choose(n, nrow(x)) > ((2^64) - 2)){ stop("(n,k) combination too big; combinadics limited to 64-bit integer arithmetic.") }
	return(rank_R(x-1L, n)+1L)
}

#' rank_grid
#' @export 
rank_grid <- function(x, dims, column_major = TRUE){
	stopifnot(!is.null(dim(x)) && nrow(x) == 2)
	stopifnot(all(x[1,] >= 1 & x[1,] <= dims[1] & x[2,] >= 1 & x[2,] <= dims[2]))
	rank_gridR(x-1L, nr = dims[1], nc = dims[2], column_major = column_major)+1L
}

#' unrank_grid
#' @export 
unrank_grid <- function(x, dims, column_major = TRUE){
	stopifnot(all(x >= 1 & x <= prod(dims)))
	unrank_gridR(x-1L, nr = dims[1], nc = dims[2], column_major = column_major)+1L
}
...
#' rank_szudsick
#' @export
rank_szudsick <- function(x){
	stopifnot(!is.null(dim(x)) && nrow(x) == 2)
	{ a <- x[1,] - 1L; b <- x[2,] - 1L }
	ifelse(a >= b, a * a + a + b, a + b * b) + 1L
}

#' unrank_szudsick
#' @export
unrank_szudsick <- function(x){
	stopifnot(is.numeric(x) && is.vector(x))
	.unrank <- function(z){	
		{ sqrtz <- floor(sqrt(z)); sqz <- sqrtz * sqrtz }
  	if ((z - sqz) >= sqrtz){ c(sqrtz, z - sqz - sqrtz) } else { c(z - sqz, sqrtz) }
  }
  sapply(x-1L, .unrank)+1L
}
